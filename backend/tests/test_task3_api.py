...
def test_community_post_crud_list_and_detail() -> None:
    with _build_client() as client:
        author_id = str(uuid.uuid4())

        create_user = client.put(
            f"/api/v1/users/{author_id}/profile",
            json={
        ...
        create_post = client.post(
            ...
            json={
                ...
                "title": "东京夜景攻略",
                "content": "分享几个适合拍照的夜景点位。",
                "status": "draft",
            },
        )
        assert create_post.status_code == 201
        post_json = create_post.json()["data"]
        post_id = post_json["id"]
        assert post_json["published_at"] is None

        detail_response = client.get(f"/api/v1/community-posts/{post_id}")
        assert detail_response.status_code == 200
        assert detail_response.json()["data"]["title"] == "东京夜景攻略"

        list_response = client.get("/api/v1/community-posts", params={"author_user_id": author_id})
        assert list_response.status_code == 200
        assert len(list_response.json()["data"]["items"]) == 1

        publish_response = client.patch(
            f"/api/v1/community-posts/{post_id}",
            json={"status": "published", "like_count": 12},
        )
        ...
        assert publish_data["like_count"] == 12

        delete_response = client.delete(f"/api/v1/community-posts/{post_id}")
        assert delete_response.status_code == 204

        missing_response = client.get(f"/api/v1/community-posts/{post_id}")
